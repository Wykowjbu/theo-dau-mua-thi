    ...
    <Card className="h-full" variant={featured ? "tertiary" : "default"}>
      ...
      <Card.Content className="space-y-6">
        <dl className={`grid gap-4 ${featured ? "md:grid-cols-2" : ""}`}>
          ...
          <div><dt className="mb-1 flex items-center gap-2 text-xs font-bold uppercase tracking-widest text-muted"><Person width={15} height={15} aria-hidden="true" />Thí sinh nào?</dt><dd>Nguồn chỉ nói <strong>một số thí sinh</strong>; website không công bố danh tính hay số báo danh.</dd></div>
        </dl>
        <div>
          <p className="mb-2 flex items-center gap-2 text-xs font-bold uppercase tracking-widest text-muted"><ScalesBalanced width={15} height={15} aria-hidden="true" />Cách vi phạm được công bố</p>
