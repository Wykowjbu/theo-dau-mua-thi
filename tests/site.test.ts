...
test("hồ sơ Quảng Trị chỉ giữ vai trò ẩn danh, cáo buộc và chứng cứ được cung cấp", () => {
  const incident = incidents.find((item) => item.id === "quang-tri-threads-can-thiep-bai-thi-2026");
  assert.ok(incident);
  assert.equal(incident.overallStatus, "UNVERIFIED");
  assert.equal(incident.people.length, 0);
  assert.ok(incident.quickEvidenceFactIds.length > 0);
  assert.ok(incident.quickEvidenceFactIds.every((id) => incident.keyFacts.some((fact) => fact.id === id)));
  assert.doesNotMatch(JSON.stringify(incident), /NHTA|TLBN|Hướng Hóa|@/i);
});
